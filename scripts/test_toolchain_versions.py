...
class ToolchainVersionContractTests(unittest.TestCase):
    ...
    def test_shared_pnpm_install_retry_is_bounded_and_fail_closed(self):
        ...
        self.assertRegex(action, r'(?m)^  skip-skia-download:
...
if __name__ == "__main__":
    unittest.main()
        ...
        self.assertRegex(action, r'(?m)^    default: "true"
    def test_pnpm_version_management_is_external_and_lockfile_stays_single_document(self):
        workspace = read("pnpm-workspace.yaml")
        npmrc = read(".npmrc")
        lockfile = read("pnpm-lock.yaml")
...
if __name__ == "__main__":
    unittest.main()
        ...
        self.assertIn("SKIP_SKIA_DOWNLOAD: ${{ inputs.skip-skia-download }}", action)
