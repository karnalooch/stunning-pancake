...
| ID | Transza | Priorytet | Status | Gałąź | Zależności |
|----|---------|-----------|--------|-------|------------|
| T00 | Publikacja master planu | P0 | PLANNED | docs/takeover-cleanup-plan | - |
| T01 | Signing key removal + ci guard | P0 | PLANNED | security/signing-key-removal | kontynuacja #47 |
...
| T21 | Mobile CI filter + test integrity | P0 | PLANNED | ci/mobile-filter-and-tests | - |
...
3. T22 CI path routing + aggregate check (pierwszy, bo inne transze zależą od wiarygodnego CI).
4. T21 mobile filter + test integrity.
...
### Scope
- `D:\gem\stunning-pancake\backend\railway.json` – usunięcie `SECRET_KEY`, zachowanie struktury z placeholderem referencyjnym.
- `D:\gem\stunning-pancake\backend\celery-worker\railway.json`, `backend\celery-worker-simulation\railway.json`, `celery-worker-routing\railway.json`, `admin\railway.json`, `telemetry\railway.json` – audyt tych samych kluczy.
- `D:\gem\stunning-pancake\scripts\check_config_secrets.py` (śledzony w `docs/operations/SIGNING_KEY_ROTATION.md`, ale nieobecny w `git ls-files`) – dodanie skryptu z testami.
...
- Skrypt ma co najmniej 4 testy jednostkowe (placeholder akceptowany, wartość nieplaceholder odrzucana, klucz spoza listy odrzucany, brak pola akceptowany).
...
### Dependencies
- Kontynuacja otwartego PR #47 (jeżeli jeszcze aktywny); w przeciwnym razie nowa gałąź `security/signing-key-removal`.
- Właściciel potwierdza, że Railway ma niezależny klucz.
...
2. Utwórz gałąź `security/signing-key-removal` z aktualnego main.
...
4. Dodaj `scripts/check_config_secrets.py` z testami w `scripts/test_check_config_secrets.py`:
   - placeholder akceptowany, wartość nieplaceholder odrzucana, nieznany klucz odrzucany, brak pola akceptowany.
   - ścieżki z `git ls-files` dla `railway.json`, wyłącznie raportując `path:field` bez wartości.
...
Zasady:
- Bez `git reset --hard`, `git push --force`.
- Bez globalnego formatowania, bez aktualizacji zależności, bez zmian w dokumentacji.
- Bez ujawniania wartości sekretów w diff, logach ani PR.
...
Nie akceptuj rozwiązań, które maskują wartość, pozostawiają `SECRET_KEY` w pliku lub
wyłączają bramkę bezpieczeństwa.
```
