...
def generate_loop_gps(center_lon, center_lat, radius_km=1.5, jitter=0.0003, num_points=50):
    ...
    for i in range(num_points):
        ...
        dlon = (
            (r * math.sin(angle))
            / (earth_radius_km * math.cos(math.radians(center_lat)))
            * (180 / math.pi)
        )
...
def generate_out_and_back_gps(
    start_lon, start_lat, bearing_deg, distance_km=3.0, jitter=0.0002, num_points=40
):
    ...
    for i in range(num_points):
        ...
        dlon = (
            (d * math.sin(bearing_rad))
            / (earth_radius_km * math.cos(math.radians(start_lat)))
            * (180 / math.pi)
        )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--clear", action="store_true", help="Clear existing seed activities before seeding"
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["clear"]:
            deleted, _ = Activity.objects.filter(user__username__startswith="athlete_").delete()
        ...
        athlete_configs = [
            ...
            (
                ...
                "Katarzyna",
                "Lewandowski",
                "katarzyna.lewandowski@example.com",
                siedlce,
            ),
        ...
        for username, first_name, last_name, email, tenant in athlete_configs:
            ...
            if created:
                ...
                user.save()
                self.stdout.write(
                    f"  Created: {username} ({first_name} {last_name}) @ {tenant.name}"
                )
        ...
        if existing > 0 and not options["clear"]:
            self.stdout.write(
                self.style.WARNING(
                    f"{existing} activities already exist. Use --clear to regenerate."
                )
            )
        ...
        poi_configs = [
            (
                ...
                SIEDLCE_CENTER,
                "COFFEE",
                "Specialty coffee with athlete discounts",
                siedlce,
            ),
            (
                "Bike Zone Siedlce",
