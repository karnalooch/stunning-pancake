...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if orphaned_participations.exists():
            self.stdout.write(self.style.WARNING(f"Found {orphaned_participations.count()} orphaned participations!"))
            for p in orphaned_participations:
                self.stdout.write(self.style.ERROR(f"  Participation ID: {p.id} points to non-existent User ID: {p.user_id}"))
        else:
        ...
        if inconsistent_tenants > 0:
            self.stdout.write(self.style.WARNING(f"Found {inconsistent_tenants} inconsistent tenant assignments."))
        else:
