...
def find_or_create_oauth_user(
    *,
    ...
    admin_emails = [e.strip().lower() for e in os.getenv('GLOBAL_OWNER_EMAILS', '').split(',') if e.strip()]
    ...
    user.save()
    logger.info('oauth.%s user_id=%s created=%s provider_id=%s', provider, user.id, created, provider_id)
    return user, created
