...
FROM_EMAIL = os.getenv('FROM_EMAIL', 'no-reply@sport-platform.com')
FROM_NAME = os.getenv('FROM_NAME', 'SPORT Platform')
...
class EmailService:
    ...
    def send_password_reset(cls, to_email: str, reset_token: str) -> bool:
        ...
        <div style="font-family: monospace; max-width: 600px; margin: 0 auto; padding: 20px; background: #0B1D33; color: #F5E6CC;">
            ...
            <p style="font-size: 10px; color: #8B7355;">SPORT Platform — AntiGravity Systems</p>
        </div>
        """
    ...
    def send_invitation(cls, to_email: str, username: str, temp_password: str, tenant_name: str = '') -> bool:
        subject = f"SPORT Platform — Invitation{f' to {tenant_name}' if tenant_name else ''}"
        body = f"""
        <div style="font-family: monospace; max-width: 600px; margin: 0 auto; padding: 20px; background: #0B1D33; color: #F5E6CC;">
            <h1 style="color: #D4A373; font-size: 24px;">YOU HAVE BEEN RECRUITED</h1>
            <p>Welcome to the SPORT Platform{f' — {tenant_name}' if tenant_name else ''}.</p>
    ...
    def send_beta_acknowledgment(cls, to_email: str, category: str) -> bool:
        subject = "SPORT — Beta Feedback Received"
        body = f"""
        <div style="font-family: monospace; max-width: 600px; margin: 0 auto; padding: 20px; background: #0B1D33; color: #F5E6CC;">
