...
class Club(models.Model):
    ...
    sport_type = models.CharField(max_length=10, choices=SPORT_TYPES, default="MIXED")
...
class ClubChallenge(models.Model):
    ...
    challenger = models.ForeignKey(Club, on_delete=models.CASCADE, related_name="challenges_issued")
    opponent = models.ForeignKey(Club, on_delete=models.CASCADE, related_name="challenges_received")
    ...
    def compute_normalized_score(self, club: Club) -> float:
        ...
        if active_count == 0:
            return 0.0
        total_km = members.aggregate(total=models.Sum("total_km"))["total"] or 0.0
