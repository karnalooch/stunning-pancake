...
def _table_sequences(table: str) -> list[str]:
    rows = _fetchall(
...
def _rls_test_role_setup(django_db_setup, django_db_blocker):
    ...
    ``postgis/postgis`` service image). Idempotent: drops any pre-existing
    role before recreating, and disconnects any active sessions before
    both ``DROP ROLE`` calls so a previous aborted run cannot mask this
    one. Teardown never raises into the test result.
    """
    _require_postgres()
    with django_db_blocker.unblock():
        ...
        _exec(f"DROP ROLE IF EXISTS {TEST_ROLE}")
        _exec(f"CREATE ROLE {TEST_ROLE} NOLOGIN NOSUPERUSER NOBYPASSRLS")
        for table in PROTECTED_TABLES:
            _exec(f"GRANT SELECT, INSERT, UPDATE, DELETE ON {table} TO {TEST_ROLE}")
        for table in PROTECTED_TABLES:
            for seq in _table_sequences(table):
                _exec(f"GRANT USAGE, SELECT ON SEQUENCE {seq} TO {TEST_ROLE}")
    yield
    with django_db_blocker.unblock():
        _exec(
            "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE usename = %s",
        ...
        for table in PROTECTED_TABLES:
            _exec(f"REVOKE ALL PRIVILEGES ON {table} FROM {TEST_ROLE}")
            for seq in _table_sequences(table):
                _exec(f"REVOKE ALL PRIVILEGES ON SEQUENCE {seq} FROM {TEST_ROLE}")
        _exec(f"DROP ROLE IF EXISTS {TEST_ROLE}")
