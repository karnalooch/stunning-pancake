...
def _resolve_host(host: str) -> list[ipaddress._BaseAddress]:
    ...
    if not host:
        raise UnsafeWebhookURL("URL is missing a host.")
    ...
    try:
        ...
        raise UnsafeWebhookURL(f"Hostname could not be resolved: {host}") from exc
    ...
    if not addrs:
        raise UnsafeWebhookURL(f"Hostname resolved to no addresses: {host}")
    return addrs
...
def validate_outbound_url(url: Any) -> str:
    ...
    Raises ``UnsafeWebhookURL`` with a short reason on any failure.
    """

    if not isinstance(url, str) or not url:
        raise UnsafeWebhookURL("URL must be a non-empty string.")

    try:
        parts = urlsplit(url)
    except ValueError as exc:
        raise UnsafeWebhookURL(f"Malformed URL: {exc}") from exc

    scheme = (parts.scheme or "").lower()
    if scheme != "https":
        raise UnsafeWebhookURL(f"Only https URLs are allowed (got {scheme!r}).")

    if parts.username or parts.password:
        raise UnsafeWebhookURL("URLs with userinfo are not allowed.")
    ...
    if unsafe:
        sample = ", ".join(str(ip) for ip in unsafe[:3])
        raise UnsafeWebhookURL(f"Host {host!r} resolves to non-public address(es): {sample}.")
    return url
