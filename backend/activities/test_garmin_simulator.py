...
class ScheduleGenerationTests(SimpleTestCase):
    ...
    def test_start_near_siedlce(self):
        ...
        for plan in plans:
            dist = _haversine_m(
                SIEDLCE_CENTER[0], SIEDLCE_CENTER[1], plan.start_lat, plan.start_lon
            )
        ...
        pts = [
            {
                ...
                "cad": 80,
                "atemp": 22,
                "time": plan.start_time.isoformat(),
                "speed_kmh": 25,
            },
            {
                "tick": 1,
                ...
                "cad": 82,
                "atemp": 22,
                "time": (plan.start_time + timedelta(seconds=1)).isoformat(),
                "speed_kmh": 25,
            },
        ]
...
class _FakeRedis:
    ...
    def ltrim(self, key, start, stop):
        if key in self.lists:
            self.lists[key] = (
                self.lists[key][start : stop + 1] if stop != -1 else self.lists[key][start:]
            )
...
class RedisStateTests(TestCase):
    def setUp(self):
        ...
        self.get_redis_patcher = patch(
            "activities.garmin_simulator.get_redis", return_value=self.fake_redis
        )
...
class GarminLiveSimulatorTests(TestCase):
    ...
    def test_schedule_rides_success(self, mock_apply_async):
        from activities.garmin_simulator import ScheduleConfig, schedule_rides
    ...
    def test_run_live_tick_0(self, mock_apply_async, mock_push):
        ...
        from activities.garmin_simulator import RidePlan, run_live_tick, set_ride_state
