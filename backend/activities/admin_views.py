...
class LiveSimulationView(APIView):
    ...
    def post(self, request):
        ...
        if state["running"]:
            return Response(
                {"error": "Live simulation already running."}, status=status.HTTP_409_CONFLICT
            )
...
class WipeDataView(APIView):
    ...
    def delete(self, request):
        ...
        if not confirm:
            return Response(
                {"error": "Must send ?confirm=true"}, status=status.HTTP_400_BAD_REQUEST
            )
            ...
            if stats:
                for worker_name, worker_stats in stats.items():
                    ...
                    worker_info = {
                        ...
                        if isinstance(worker_stats.get("pool"), dict)
                        else 0,
