...
class SocialSharingService:
    ...
    def generate_activity_card_data(cls, activity):
        ...
        return {
            ...
            "avg_speed": round(
                (activity.distance / 1000.0) / (activity.duration.total_seconds() / 3600.0), 2
            )
            if activity.duration and activity.duration.total_seconds() > 0
            else 0,
            "date": activity.start_time.strftime("%d %b %Y"),
            "is_verified": activity.is_verified,
            "tenant_name": activity.user.tenant_id
            if activity.user.tenant_id
            else "Independent Athlete",
        }
