...
from users.permissions import HasPermission, HasAnyPermission, IsGlobalOwner, IsTenantAdmin, IsAdminOrModerator
...
def tenant_admin(db, tenant):
    return User.objects.create_user(username='ta', password='pass', role='TENANT_ADMIN', tenant=tenant)

@pytest.fixture
def tenant_moderator(db, tenant):
    return User.objects.create_user(username='tm', password='pass', role='TENANT_MODERATOR', tenant=tenant)
...
def rbac_permissions(db):
    ...
    for codename, name, resource, action in perms:
        p, _ = Permission.objects.get_or_create(codename=codename, defaults={'name': name, 'resource': resource, 'action': action})
        created.append(p)
    return created
...
class TestPermissionModel:
    def test_unique_codename(self, rbac_permissions):
        with pytest.raises(Exception):  # IntegrityError
            Permission.objects.create(codename='activities.view', name='Dup', resource='x', action='x')

    def test_unique_resource_action(self, rbac_permissions):
        with pytest.raises(Exception):
            Permission.objects.create(codename='unique.code', name='Dup', resource='activities', action='view')
