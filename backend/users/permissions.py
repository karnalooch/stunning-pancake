...
class IsGlobalOwner(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        return request.user.is_authenticated and getattr(request.user, 'role', None) == 'GLOBAL_OWNER'
...
class IsAdminOrModerator(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        role = getattr(request.user, 'role', None)
        return request.user.is_authenticated and role in ('GLOBAL_OWNER', 'TENANT_ADMIN', 'TENANT_MODERATOR')
