    ...
    def handle(self, *args, **options):
        ...
        for codename, name, resource, action, description in PERMISSIONS:
            perm, _ = Permission.objects.get_or_create(
                codename=codename,
                defaults={'name': name, 'resource': resource, 'action': action, 'description': description}
            )
        ...
        for slug, name in Role.SLUG_CHOICES:
            role, _ = Role.objects.get_or_create(
                slug=slug,
                defaults={'name': name, 'description': f'System role: {name}'}
            )
            # Assign permissions
            ...
            self.stdout.write(self.style.SUCCESS(f'  Role "{slug}" with {len(perm_codenames)} permissions'))
