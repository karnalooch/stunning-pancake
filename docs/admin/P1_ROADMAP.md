...
| 4 | Live sim: `ride_warming` → spadek; `routing_queue_depth` stabilne przy `SCALE_SIM_MAX_ROUTING_QUEUE_DEPTH` | 🟡 w trakcie (2026-06-04) — **`active_ratio` obniżony 0.46→0.18**, `tick_seconds=8`, caps 30/25/4 (domyślne). Routing **2 repliki × 512 MB**. Po restarcie sim ramp-up: `ride_warming` spada po rozgrzaniu (obserwacja ≥15 min). `routing_queue_depth` stabilny przy cap w fazie ramp-up — pełny spadek backpressure w toku |
...
| 6 | Logi prod: brak lawiny `sim.routing.backpressure` >15 min przy normalnym `active_ratio` | 🟡 w trakcie (2026-06-04) — **naprawiono przyczynę:** `active_ratio` było **0.46** (popyt >> drenaż). Obniżono do **0.18** + routing 2 repliki + domyślne caps. Backpressure oczekiwany w fazie ramp-up (~5–10 min po restarcie); **weryfikacja ≥15 min stabilności** po rozgrzaniu — operator: `railway logs -s celery-worker-simulation \| Select-String backpressure` |
