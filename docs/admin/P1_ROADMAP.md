...
| 4 | Live sim: `ride_warming` → spadek; `routing_queue_depth` stabilne przy `SCALE_SIM_MAX_ROUTING_QUEUE_DEPTH` | 🟡 **nie domknięte** (2026-06-04, final attempt) — `active_ratio` **0.12→0.10** + restart API; po ~9 min: `running=True`, **`ride_warming=88`** (bez spadeku), **`routing_queue_depth=88`** (> cap 80), brak trendu w dół. Kolejny krok: dalsze obniżenie ratio / cap / więcej czasu po restarcie |
...
| 6 | Logi prod: brak lawiny `sim.routing.backpressure` >15 min przy normalnym `active_ratio` | 🟡 **nie domknięte** (2026-06-04, final attempt) — `railway logs -s celery-worker-simulation --since 10m`: **~73** wpisy backpressure; ostatnie 5 min **~36** (~co tick w ramp-up). Przy `active_ratio=0.10` nadal lawina; brak 15 min ciszy — **bez ✅** |
