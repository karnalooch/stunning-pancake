...
| **canonical_path** | docs/en/operations/TELEMETRY_INGEST_QUEUE.md |
...
Automatic: the drain loop runs `XAUTOCLAIM` every `TELEMETRY_INGEST_RECLAIM_INTERVAL_S` (default 30s) for entries idle longer than `TELEMETRY_INGEST_PEL_IDLE_MS` (default 60s).
...
If `TELEMETRY_OPS_SECRET` is unset, the reclaim endpoint is open (staging only — set the secret in prod).

## DLQ replay (manual)

1. Inspect DLQ: `XRANGE telemetry:ingest:dead - + COUNT 10`
2. Fix root cause (schema, DB pool, bad payload).
3. Copy the `data` field back to the main stream with `XADD` or re-post via HTTP ingest/backfill.
