...
| **lang** | en |
| **translation** | [Polski](../EVENT_BURST_50K.md) |
| **canonical_path** | docs/en/EVENT_BURST_50K.md |
---
...
- **Telemetry ingest** — the FastAPI telemetry service (`telemetry/main.py`) enforces the same per-second cap on `/api/telemetry/ingest` and `/ingest/batch`, returning `429 + Retry-After` always (not event-gated). The Django simulator publish path feeds the same global ingest signal.```env
...
## Environment variables (optional tuning)```env
