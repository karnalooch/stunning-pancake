...
- [ ] **Impersonation Endpoint**:
  - Create `/api/auth/impersonate/` view in `users.views.py`.
  - Restrict access strictly to users with `GLOBAL_OWNER` role.
  - Return a short-lived JWT for the target user with an `impersonated: true` claim.
- [ ] **Audit Logs**:
  - Create an `AuditLog` model to track sensitive actions.
  - Add middleware or decorators to log actions performed while an impersonation token is active.
...
- [ ] **Core Auth Architecture (`admin/src/core/auth/`)**:
  - Implement `useAuth` hook (via Zustand or Context API) to manage the JWT payload, current user role, and tenant feature flags.
- [ ] **Route Guards (`admin/src/core/guards/`)**:
  - Create a `<RoleGuard requiredRole={['GLOBAL_OWNER', 'TENANT_ADMIN']}>` wrapper component to protect routes.
- [ ] **Module Reorganization**:
  - Move global views (e.g., `Dashboard`, `Tenants`) into `admin/src/modules/global-admin/`.
  - Create views tailored for Tenant Admins (e.g., `admin/src/modules/tenant-admin/`).
  - Update the Sidebar navigation to dynamically render items based on `user.role` and `tenant.has_heatmap_analytics`.
...
- [ ] **Unit Tests**:
  - Write tests for `permissions.py` to verify tenant isolation.
  - Test the `TenantMiddleware` and PostgreSQL RLS behavior.
- [ ] **E2E Validation**:
  - Log in as `TENANT_ADMIN` and verify that `GLOBAL_OWNER` routes return 403.
  - Use the Impersonation feature from the UI to verify audit logging.
