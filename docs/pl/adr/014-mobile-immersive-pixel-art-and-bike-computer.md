...
2. **Strefy**: skupienia (Active Ride — minimum animacji) vs zaangażowania (Home, Ride Summary + karta do share, City Hub, Profile/Bike Garage, Onboarding, Marketplace).
...
7. **Pipeline assetów**: Gemini + deterministyczny post-process (palette-quant, nearest-neighbor, sprite packing, `pngquant`/`oxipng`, manifest z seed/hash, reference-locked klatki).
