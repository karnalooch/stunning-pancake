...
5. **Paleta scena vs chrome**: sceny mogą mieć bogatszą paletę; chrome UI trzyma runtime tokeny Grand Prix; styk reguluje `scrim`.
6. **Jedno źródło tokenów**: `mobile/src/theme/grandPrix.ts` (+ punktowe `grandPrixNight`), bez legacy octopath/solar i bez legacy runtime stitch.
