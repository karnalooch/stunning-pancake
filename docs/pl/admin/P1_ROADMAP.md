...
| **lang** | pl |
| **translation** | [English](../../admin/P1_ROADMAP.md) |
| **canonical_path** | docs/pl/admin/P1_ROADMAP.md |
---
...
| **Audyt UI (snapshot)** | [UI_AUDIT_2026-06-02.md](../../admin/UI_AUDIT_2026-06-02.md) |
...
#### Operational closure (przed Paczką 2 Sponsor)| # | Kryterium | Status |
|---|-----------|--------|
...
## 8. Historia| Dane | Zmiana |
|------|------------|
| 2026-06-04 | **Zamknięcie operacyjne (Railway 8GB):** `active_ratio` 0,46 → 0,18, routing 3 → 2 repliki, RAM budżet SSOT, serwis `telemetry` rządzi, map benchmark `detail=summary`, `set_live_active_ratio` cmd + `railway-set-live-active-ratio.ps1` |
| 2026-06-04 | ** Fragmentowanie telemetrii, faza 2:** `TelemetryShardRouter.client_for()` + `REDIS_TELEMETRY_SHARD_NODES`, rozkładanie odczytu równoległego, pula/wsadowa asyncpg FastAPI, szkielet testu obciążenia ([TELEMETRY_LOAD_TEST.md](../../operations/TELEMETRY_LOAD_TEST.md)). Prod: `TELEMETRY_SHARD_COUNT=4` na backendie + symulacja |
| 2026-06-03 | **Zawsze włączona ochrona + fragmentowanie telemetrii (podstawa):** globalny `core/load_guard.py` (zawsze włączony, sterowany obciążeniem, histereza, otwarcie awaryjne — dołączenie/sesja/pobranie + ograniczenie współbieżności), `TelemetryShardRouter` (`activities/telemetry_shard.py`, shard po `deviceId`/CRC32, hash-tag `{tel:i}`, przeczytaj rozwinięcie, `TELEMETRY_SHARD_COUNT=1` = starsze), zawsze włączone przeciwciśnienie telemetryczne (sygnał Django + FastAPI 429), 32 testy. Wiele instancji na fragment = Faza 2 (patrz [operations/TELEMETRY_SHARDING.md](../../operations/TELEMETRY_SHARDING.md)) |
| 2026-06-03 | Skalowanie routingu: `numReplicas=2→3` (poziomo, łącznie ~3 GB). Pomiar prod: drenaż ~1,89/s (~liniowy), przeciwciśnienie nadal przypięte (1/6,2 s) → określonea dźwignia = wyprowadzone `active_ratio`; s.6 niezamknięte. Brak OOM |
| 2026-06-03 | Skalowanie routingu: `selery-worker-routing` `numReplicas=1→2` (poziomo, ~1 ​​GB/replikę) w odpowiedzi na utrzymujące się przeciwciśnienie; aktualizacja s.4 i s.6 zamknięcie operacyjne |
| 2026-06-03 | Weryfikacja zamknięcia operacyjnego prod (tylko do odczytu): s. 3 🟡 (infra PASS, podręcznik roli UI), s. 4 🟡 (stabilność kolejki, rozgrzewanie n/a), s. 5 ✅ (szablon + migawka), s. 6 ☐ (przeciwciśnienie ciągłe ≥7,5 min) |
| 2026-06-03 | Cross-link: pełna lista kontrolna GPX w P2 §2.3 (F1–F6) |
| 2026-06-03 | GPX → P2_ROADMAP; wyjaśnienie P2 vs Paczka 6 |
| 2026-06-03 | Przedsiębiorstwo Paczka 1b: mapa FSM, kolejka symulatora KPI, lista kontrolna bramek operacyjnych |
| 2026-06-03 | Paczka 1b: przeciwciśnienie routingu + symulator Dashboardu KPI |
| 2026-06-03 | Paczka 1a gotowa; link zweryfikuj skrypt; 1b + Paczka 2 dalej |
| 2026-06-03 | Aktualizacja dokumentu; FSM + kolejka routingu + API/UI |
