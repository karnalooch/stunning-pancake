...
3. Zainstaluj zależności z **root** monorepo: `pnpm install --frozen-lockfile` (wymaga [pnpm](https://pnpm.io/) 9.x; bez globalnego pnpm: `npx pnpm@12.4.2 install --frozen-lockfile`). **Nie** używaj `npm install` w `admin/` — `workspace:*` wymaga pnpm.
