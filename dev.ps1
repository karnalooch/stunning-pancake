function Start-SportDev {
    Write-Host "🚀 SPORT Dev Engine: Inicjalizacja..." -ForegroundColor Cyan
    ...
    while($true) {
        ...
        if ($currentHash -ne $lastHash) {
            ...
            # Apply K8s
            Write-Host "☸️ Odświeżam Kubernetes..."
            kubectl apply -k infrastructure/kubernetes/base/
            kubectl rollout restart deployment sport-backend sport-owner
            
            $lastHash = $currentHash
            Write-Host "✅ System gotowy. Czekam na zmiany..." -ForegroundColor Green
        }
